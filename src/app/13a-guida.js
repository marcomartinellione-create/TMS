...
function guidaCompleta(){
  ...
  return `
   ...
   <ol style="margin:0 0 6px 18px;line-height:1.8">
     ...
     <li><b>Il cliente compila</b> — salva il file sul telefono e lo carica nell'app con <b>📂 Carica la scheda</b> (resta memorizzata: la ritrova a ogni apertura, con la <b>bozza salvata da sola</b> mentre compila). L'app ha due sezioni: <b>🏋 Scheda allenamento</b> e <b>🍖 Alimentazione</b> (il piano in sola lettura, coi grammi in evidenza). Ogni giornata è una <b>seduta in tre fasi</b> (dal v2.0 dell'app): <b>🔥 Riscaldamento</b> (se lo hai messo nel Training Set: sola lettura coi video, non torna indietro), <b>🏋 Esercizi</b> e <b>✅ Fine</b>. Nella fase Esercizi tocca una riga e l'esercizio si apre <b>a schermo pieno</b> (numeri grandi, video, timer, avanti/indietro tra un esercizio e l'altro); sotto ogni esercizio compare anche <b>«l'ultima volta»</b> (cosa ha fatto la volta scorsa, dallo Storico). Compila i valori effettivi (serie, ripetizioni, peso, RIR e note; fatica RPE 0–10 e durata in minuti nella fase Fine; il <b>RIR parte vuoto</b>). Se la scheda è <b>modificabile</b>, può anche <b>aggiungere, eliminare e modificare</b> gli esercizi e segnare i <b>test del massimale (★)</b>. Poi preme <b>📩 Crea il file per il coach</b> (vede un <b>riepilogo</b> di quanto ha fatto): nasce <span class="mono">Rientro_&lt;profilo&gt;_&lt;data&gt;.json</span> da rimandarti (dove possibile con la condivisione diretta su WhatsApp). Il riscaldamento è solo informativo e <b>non entra nel rientro</b>.</li>
     <li><b>Importa</b> — nella riga del cliente, <b>📥 Importa rientro</b>: scegli il file e l'allenamento del cliente viene <b>caricato nella scheda 🏋 Pesi</b> (esercizi, valori effettivi e fatica/durata delle sedute; gli esercizi <b>aggiunti dal cliente</b> compaiono qui — se fuori catalogo, con un avviso — e i <b>test del massimale ★</b> arrivano già segnati). Lì fai un <b>controllo generale</b>, correggi se necessario, poi <b>💾 Salva nello Storico</b> scegliendo tu la settimana: solo allora entra nello Storico (con sedute e RPE) e alimenta TL, ACWR, grafici e Report. Niente viene scritto automaticamente.</li>
   </ol>
...
function guidaCompletaEN(){
  ...
  return `
   ...
   <ol style="margin:0 0 6px 18px;line-height:1.8">
     ...
     <li><b>The client fills it in</b> — they save the file on the phone and load it into the app with <b>📂 Load the program</b> (it stays stored: they find it at every opening, with the <b>draft auto-saved</b> while filling in). The app has two sections: <b>🏋 Training program</b> and <b>🍖 Nutrition</b> (the read-only plan, with grams highlighted). Each day is a <b>session in three phases</b> (from app v2.0): <b>🔥 Warm-up</b> (if you added it to the Training Set: read-only with videos, doesn't come back), <b>🏋 Exercises</b> and <b>✅ Done</b>. In the Exercises phase they tap a row and the exercise opens <b>full-screen</b> (big numbers, video, timer, prev/next between exercises); under each exercise there's also <b>«last time»</b> (what they did last time, from History). They fill in the actual values (sets, reps, weight, RIR and notes; RPE effort 0–10 and duration in the Done phase; the <b>RIR starts empty</b>). If the program is <b>editable</b>, they can also <b>add, remove and edit</b> exercises and mark the <b>max tests (★)</b>. Then they press <b>📩 Create the file for the coach</b> (they see a <b>summary</b> first): it creates <span class="mono">Rientro_&lt;profile&gt;_&lt;date&gt;.json</span> to send back to you (where possible with direct WhatsApp sharing). The warm-up is informational only and <b>doesn't enter the log</b>.</li>
     <li><b>Import</b> — on the client's row, <b>📥 Import log</b>: pick the file and the client's workout is <b>loaded into the 🏋 Weights program</b> (exercises, actual values and session effort/duration; exercises <b>added by the client</b> show up here — with a notice if off-catalog — and the <b>max tests ★</b> arrive already marked). There you do a <b>general check</b>, fix if needed, then <b>💾 Save to History</b> choosing the week yourself: only then it enters History (with sessions and RPE) and feeds TL, ACWR, charts and Report. Nothing is written automatically.</li>
   </ol>
