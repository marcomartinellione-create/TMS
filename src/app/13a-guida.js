...
function guidaCompleta(){
  ...
  return `
   ...
   <ol style="margin:0 0 6px 18px;line-height:1.8">
     ...
     <li><b>Il cliente compila</b> — salva il file sul telefono e lo carica nell'app con <b>📂 Carica la scheda</b> (resta memorizzata: la ritrova a ogni apertura, con la <b>bozza salvata da sola</b> mentre compila). L'app ha due sezioni: <b>🏋 Scheda allenamento</b> e <b>🍖 Alimentazione</b> (il piano in sola lettura, coi grammi in evidenza). Nella scheda sceglie il <b>giorno</b> da un elenco, compila i valori effettivi (serie, ripetizioni, peso, RIR e note per esercizio; fatica RPE 0–10 e durata in minuti per seduta; il <b>RIR parte vuoto</b>: lo inserisce dopo l'allenamento, il previsto resta indicato sopra), guarda i ▶ video e usa il <b>timer di recupero</b> (bottone ⏱ su ogni esercizio, parte dal tempo di riposo scritto nella scheda — disponibile sempre). Se hai esportato una scheda <b>modificabile</b>, può anche <b>aggiungere, eliminare e modificare</b> gli esercizi (rinomina, sposta di giorno, ritocca il previsto) e segnare i <b>test del massimale (★)</b>; in una scheda <b>fissa</b> compila e basta. Poi preme <b>📩 Crea il file per il coach</b>: nasce <span class="mono">Rientro_&lt;profilo&gt;_&lt;data&gt;.json</span> da rimandarti (dove possibile con la condivisione diretta su WhatsApp).</li>
...
function guidaCompletaEN(){
  ...
  return `
   ...
   <ol style="margin:0 0 6px 18px;line-height:1.8">
     ...
     <li><b>The client fills it in</b> — they save the file on the phone and load it into the app with <b>📂 Load the program</b> (it stays stored: they find it at every opening, with the <b>draft auto-saved</b> while filling in). The app has two sections: <b>🏋 Training program</b> and <b>🍖 Nutrition</b> (the read-only plan, with grams highlighted). In the program they pick the <b>day</b> from a list, fill in the actual values (sets, reps, weight, RIR and notes per exercise; RPE effort 0–10 and duration in minutes per session; the <b>RIR starts empty</b>: they enter it after training, the plan stays shown above), watch the ▶ videos and use the <b>rest timer</b> (⏱ button on each exercise, starts from the program's rest time — always available). If you exported an <b>editable</b> program, they can also <b>add, remove and edit</b> exercises (rename, move to another day, tweak the plan) and mark the <b>max tests (★)</b>; in a <b>fixed</b> program they just fill it in. Then they press <b>📩 Create the file for the coach</b>: it creates <span class="mono">Rientro_&lt;profile&gt;_&lt;date&gt;.json</span> to send back to you (where possible with direct WhatsApp sharing).</li>
