...
async function init(){
  ...
  { const bl=document.getElementById('btn-lang');
    if(bl){ bl.textContent=LANG==='it'?'EN':'IT'; bl.title=LANG==='it'?'Switch to English':'Torna all\'italiano';
      bl.onclick=()=>setLang(LANG==='it'?'en':'it'); } }
  { const bt=document.getElementById('btn-theme');
    const applyTheme=(md)=>{ document.documentElement.setAttribute('data-theme',md); if(bt){ bt.textContent=md==='notte'?'☀':'🌙'; bt.title=md==='notte'?t('Modalità giorno'):t('Modalità notte'); } };
  ...
  { const sg=document.getElementById('fab-segnala'); if(sg) sg.onclick=segnalaModal; }
  const qr=document.getElementById('qr-ig');
  if(qr) qr.onclick=()=>{ modal(`<h3>By Wander</h3><div style="display:flex;gap:18px;flex-wrap:wrap;justify-content:center;align-items:flex-start"><div style="text-align:center;flex:0 1 auto"><div class="muted mono" style="font-size:11px;margin-bottom:5px">Instagram</div><img src="${qr.src}" alt="QR Instagram" style="width:200px;height:200px;max-width:40vw;border-radius:var(--r-sm);border:1px solid var(--border)"><div><a class="btn btn--gold" href="https://instagram.com/marco_the_wander" target="_blank" rel="noopener" style="margin-top:8px">${t('Apri Instagram ↗')}</a></div></div><div style="text-align:center;flex:0 1 auto"><div class="muted mono" style="font-size:11px;margin-bottom:5px">GitHub · TMS</div><img src="${QR_GH_SRC}" alt="QR GitHub TMS" style="width:200px;height:200px;max-width:40vw;border-radius:var(--r-sm);border:1px solid var(--border)"><div><a class="btn btn--gold" href="${GH_REPO_URL}" target="_blank" rel="noopener" style="margin-top:8px">${t('Apri GitHub ↗')}</a></div></div>${YT_URL?`<div style="text-align:center;flex:0 1 auto"><div class="muted mono" style="font-size:11px;margin-bottom:5px">Tutorial · YouTube</div><img src="${QR_YT_SRC}" alt="QR Tutorial YouTube" style="width:200px;height:200px;max-width:40vw;border-radius:var(--r-sm);border:1px solid var(--border)"><div><a class="btn btn--gold" href="${YT_URL}" target="_blank" rel="noopener" style="margin-top:8px">${t('Apri i Tutorial ↗')}</a></div></div>`:''}</div><div class="modal__actions"><button class="btn" onclick="closeModal()">${t('Chiudi')}</button></div>`); { const _m=document.getElementById('modal'); if(_m) _m.style.maxWidth=YT_URL?'840px':'560px'; } };
