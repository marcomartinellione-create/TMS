...
function foodDetail(nome,grammi){
  ...
  if(!fo) return `<span class="muted">«${esc(nome)}» ${t('non è nella banca dati: nessun dettaglio nutrienti.')}</span>`;
  const g=+grammi||0;
  let html=`<div style="font-size:12px;color:var(--ink-3);margin-bottom:7px">${t('Valori per')} <b>${nf(g,0)} g</b> ${t('di')} <b>${esc(fo.nome)}</b>${fo.categoria?` · <span class="muted">${esc(fo.categoria)}</span>`:''} <span class="muted">${t('(tra parentesi: per 100 g)')}</span></div>`;
  html+=`<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px">`;
  NUTR_FULL.forEach(([grp,items])=>{
    html+=`<div><div class="mono" style="font-size:11px;color:var(--orange);border-bottom:1px solid var(--border);margin-bottom:4px;padding-bottom:2px">${t(grp)}</div>`;
    items.forEach(([k,lab,unit,dec])=>{ const p100=fo.per100[k]; const val=p100==null?null:p100*g/100;
      html+=`<div style="display:flex;justify-content:space-between;font-size:12px;padding:1px 0"><span>${t(lab)}</span><span class="mono">${val==null?'—':nf(val,dec)} ${unit} <span class="muted">(${p100==null?'—':nf(p100,dec)})</span></span></div>`; });
    html+=`</div>`;
  });
  return html+`</div>`;
}
...
function renderFoodPicker(fase,i){
  ...
  const rowHtml=fo=>{ const fav=foodIsFav(fo.nome);
    return `<tr class="fp-row" data-nome="${esc(fo.nome)}" style="cursor:pointer">
      <td class="no-print" style="text-align:center;padding:2px;width:24px"><button class="exp-star${fav?' on':''}" data-fav="${esc(fo.nome)}" title="${fav?t('togli dai preferiti'):t('aggiungi ai preferiti')}">${fav?'★':'☆'}</button></td>
  ...
  if(q){ const list=FOOD.filter(fo=>foodMatch(fo,q)); count=list.length; body=list.slice(0,cap).map(rowHtml).join(''); }
    ...
    if(favFoods.length){ body+=grp(t('★ Preferiti'))+favFoods.map(rowHtml).join(''); }
    if(rec.length){ body+=grp(t('🕐 Recenti'))+rec.map(rowHtml).join(''); }
    if(favFoods.length||rec.length) body+=grp(t('Tutti gli alimenti'));
...
function renderAlimentazione(){
  ...
  function faseTable(fase,rows){
    ...
    meals.forEach((meal,mi)=>{
      ...
      body+=`<tr class="day-sep"><td colspan="7">▌ ${esc(t(meal))} <span style="opacity:.85;font-family:var(--font-mono);font-size:11px">· ${nf(mk,0)} kcal</span>
        <button class="btn btn--sm no-print" data-mealup data-fase="${fase}" data-meal="${esc(meal)}" title="${t('sposta pasto su')}"${mi===0?' disabled':''} style="margin-left:8px">▲</button>
        <button class="btn btn--sm no-print" data-mealdn data-fase="${fase}" data-meal="${esc(meal)}" title="${t('sposta pasto giù')}"${mi===meals.length-1?' disabled':''}>▼</button>
        <button class="btn btn--sm no-print" data-mealedit data-fase="${fase}" data-meal="${esc(meal)}" title="${t('rinomina pasto')}">✎</button>
        <button class="btn btn--sm no-print" data-mealadd data-fase="${fase}" data-meal="${esc(meal)}" title="${t('aggiungi alimento a questo pasto')}">＋</button>
        <button class="btn btn--sm btn--danger no-print" data-mealdel data-fase="${fase}" data-meal="${esc(meal)}" title="${t('elimina pasto')}">🗑</button></td></tr>`;
      idxs.forEach(i=>{ const r=rows[i]; const known=!!FOODBYNAME[String(r.alimento||'').trim()];
        body+=`<tr data-fase="${fase}" data-i="${i}">
          <td class="l"><button class="btn btn--sm no-print food-pick" data-fase="${fase}" data-i="${i}" style="max-width:240px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;${r.alimento?'':'color:var(--ink-4)'}">${r.alimento?esc(r.alimento):t('＋ scegli alimento…')}</button>${r.alimento&&!known?` <span class="pill" title="${t('non nella banca dati')}">?</span>`:''}</td>
          ...
          <td style="white-space:nowrap">${r.alimento?`<button class="btn btn--sm no-print food-det" data-fase="${fase}" data-i="${i}" title="${t('micro e macro')}">${alimDet===fase+':'+i?'▾':'▸'}</button>`:''}<button class="btn btn--sm btn--danger no-print" data-del="${fase}:${i}" title="${t('elimina alimento')}">✕</button></td></tr>`;
    ...
    const tot=faseTot(rows);
    return `<div class="tbl-wrap"><table>
      <thead><tr><th class="l">${t('Alimento')}</th><th>g</th><th>Kcal</th><th>Prot</th><th>${t('Grassi')}</th><th>Carbo</th><th class="no-print"></th></tr></thead>
      <tbody>${body||`<tr><td colspan="7" class="empty">${t('Nessun alimento. Aggiungi un pasto qui sotto.')}</td></tr>`}</tbody>
      <tfoot><tr style="background:var(--paper-3);font-weight:600"><td class="l grp-tot">${t('Totale')} ${t(FASE_LAB[fase]||fase)}</td><td></td>
        <td class="num grp-tot">${nf(tot.kcal,0)}</td><td class="num">${nf(tot.proteine,1)}</td><td class="num">${nf(tot.grassi,1)}</td><td class="num">${nf(tot.zuccheri,1)}</td><td class="no-print"></td></tr></tfoot>
      </table></div>
      <div class="row-add no-print"><button class="btn" data-add="${fase}">${t('＋ Pasto')}</button></div>`;
  }
  const omsSection=omsRenderSection();
  /* periodi alimentari registrati (per il tab Analisi) */
  const periodi=A.periodi||[];
  const perRows=periodi.map((p,i)=>`<tr><td><span class="pill">${esc(t(FASE_LAB[p.fase]||p.fase))}</span></td>
  ...
  document.querySelectorAll('#panel-alimentazione [data-add]').forEach(b=>b.onclick=()=>chiediTesto(t('Nome del pasto (es. Colazione, Pranzo, Spuntino)'),'',v=>{
  ...
  document.querySelectorAll('#panel-alimentazione [data-mealedit]').forEach(b=>b.onclick=()=>{
    ...
    chiediTesto(t('Rinomina pasto'), old==='Senza pasto'?'':old, raw=>{
  ...
  document.querySelectorAll('#panel-alimentazione [data-mealdel]').forEach(b=>b.onclick=()=>{
    ...
    if(!confirm(t('Eliminare il pasto «')+t(meal)+t('» e tutti i suoi alimenti?')))return;
  ...
  document.querySelectorAll('#panel-alimentazione [data-perdel]').forEach(b=>b.onclick=()=>{
    ...
    if(!confirm(t('Eliminare il periodo')+' '+t(FASE_LAB[p.fase]||p.fase)+' '+p.dal+' → '+p.al+'?')) return;
...
function dietaPrintHTML(){
  ...
  meals.forEach(meal=>{
    const idxs=rows.map((r,i)=>i).filter(i=>((((rows[i].pasto||'').trim())||'Senza pasto')===meal));
    let mk=0; idxs.forEach(i=>mk+=foodVal(rows[i].alimento,rows[i].grammi,'kcal'));
    body+=`<tr class="day-sep"><td colspan="7">▌ ${esc(t(meal))} · ${nf(mk,0)} kcal</td></tr>`;
  ...
  const tot=faseTot(rows); const lab=esc(t(FASE_LAB[fase]||fase));
  const head=`<thead><tr><th class="l">${t('Alimento')}</th><th>g</th><th>Kcal</th><th>Prot</th><th>${t('Grassi')}</th><th>Carbo</th><th>${t('Fibre')}</th></tr></thead>`;
...
async function printDieta(){
  const fase=faseAlimActive(); const rows=(DOC.alimentazione&&DOC.alimentazione[fase])||[];
  if(!rows.some(r=>r&&r.alimento&&String(r.alimento).trim())){ alert(t('Il piano della fase attiva è vuoto: niente da stampare.')); return; }
  if(typeof html2canvas!=='function'){ alert(t('Generatore PDF non disponibile in questa copia.')); return; }
  const btn=document.getElementById('dieta-pdf-btn'); const blab=btn?btn.innerHTML:''; if(btn){ btn.innerHTML=t('⏳ Genero PDF…'); btn.disabled=true; }
...
function registraPeriodo(){
  ...
  modal(`<h3>${t('Registra periodo alimentare')}</h3>
   <div class="callout callout--info"><div>${t('📸 Salva una <b>fotografia</b> del piano della fase scelta con le date in cui lo segui. I periodi alimentano i grafici del tab <b>📊 Analisi</b>.')}</div></div>
   <div class="row">
    <div class="field"><label>${t('Fase')}</label><select id="per-fase">${['bulk','mant','cut'].map(f=>`<option value="${f}"${f===faseAtt?' selected':''}>${t(FASE_LAB[f]||f)}</option>`).join('')}</select></div>
    <div class="field"><label>${t('Dal')}</label><input type="date" id="per-dal" value="${fmt(fa)}"></div>
    <div class="field"><label>${t('Al')}</label><input type="date" id="per-al" value="${fmt(oggi)}"></div></div>
   <div class="modal__actions"><button class="btn" onclick="closeModal()">${t('Annulla')}</button><button class="btn btn--ember" id="per-ok">${t('Registra')}</button></div>`);
  document.getElementById('per-ok').onclick=()=>{
    ...
    if(!dal||!al){ alert(t('Servono entrambe le date.')); return; }
    if(dal>al){ alert(t('La data di inizio è dopo quella di fine.')); return; }
    ...
    if(!righe.length){ alert(t('Il piano della fase')+' '+t(FASE_LAB[fase]||fase)+' '+t('è vuoto: niente da registrare.')); return; }
    ...
    if(overlap.length && !confirm(t('⚠ Le date si sovrappongono a')+' '+overlap.length+' '+t('periodo/i già registrati (nelle settimane in comune vale l\'ultimo registrato). Continuo?'))) return;
    ...
    alert(t('✔ Periodo registrato:')+' '+t(FASE_LAB[fase]||fase)+' '+dal+' → '+al+' ('+righe.length+' '+t('alimenti,')+' '+nf(kcalPiano(righe),0)+' '+t('kcal/giorno).'));
  };
}
