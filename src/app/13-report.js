...
function renderReport(){
  ...
  if(R.andamento && ag.length){ B.andamento=`<div class="rep-sec"><div class="sec">${t('Andamento del carico')}</div>
     <div class="chart-grid">
       ...
       <div class="chart-box"><h4>${t('Equilibrio volume (serie)')}</h4>${radarChart(GRUPPI.map(g=>({label:t(g),value:(last.sets[g]||0)+(g==='Cardio'?(cardioEquivSets(last.scheda)+riscEquivSets(last.scheda)):0)})),{h:230})}</div>
     </div>
