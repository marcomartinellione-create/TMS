...
function fotoDataIt(d){ const x=new Date(d); return isNaN(x)?(d||'—'):x.toLocaleDateString(LANG==='en'?'en-GB':'it-IT'); }
...
function fotoDataLabel(f){ if(!f) return '—'; const d=new Date(f.data); const dl=isNaN(d)?(f.data||'—'):d.toLocaleDateString(LANG==='en'?'en-GB':'it-IT');
  return dl+(f.tag?' · '+esc(f.tag):''); }

function aggiungiFotoModal(file){
  const oggi=new Date().toISOString().slice(0,10);
  modal(`<h3>${t('📸 Aggiungi foto')}</h3>
    <div class="row"><div class="field"><label>${t('Data')}</label><input id="fm-data" type="date" value="${oggi}"></div>
      <div class="field"><label>Tag <span class="muted" style="text-transform:none">${t('(fronte/lato/retro o libero)')}</span></label>
    ...
    <p class="muted" style="font-size:12px">${t('La foto resta <b>sul tuo PC</b> (in')} <span class="mono">${esc(SUBDIR)}/&lt;profilo&gt;/foto/</span>${t('): non viene caricata da nessuna parte.')}</p>
    <div class="modal__actions"><button class="btn" onclick="closeModal()">${t('Annulla')}</button><button class="btn btn--ember" id="fm-ok">${t('Salva foto')}</button></div>`);
...
async function salvaFoto(file, dataStr, tag){
  if(!profileDir){ alert(t('Connetti i dati prima di aggiungere foto.')); return; }
  ...
  catch(e){ alert(t('Errore nel salvataggio della foto:')+' '+e.message); logErrore('foto', e); return; }
...
function modificaFotoModal(file){ const f=(DOC.foto||[]).find(x=>x.file===file); if(!f) return;
  modal(`<h3>${t('Modifica foto')}</h3>
    <div class="row"><div class="field"><label>${t('Data')}</label><input id="fm-data" type="date" value="${esc(f.data||'')}"></div>
      <div class="field"><label>Tag</label><input id="fm-tag" list="fm-tags" value="${esc(f.tag||'')}">
        <datalist id="fm-tags"><option value="fronte"><option value="lato"><option value="retro"></datalist></div></div>
    <div class="modal__actions"><button class="btn" onclick="closeModal()">${t('Annulla')}</button><button class="btn btn--ember" id="fm-ok">${t('Salva')}</button></div>`);
...
async function eliminaFoto(file){
  ...
  if(!confirm(t('Eliminare questa foto del')+' '+fotoDataLabel(f)+t('? Operazione irreversibile.'))) return;
...
function renderFotoSezione(){
  ...
  const tabs=`<div class="bar no-print" style="gap:6px;margin-top:4px">
    <button class="btn${fotoMode==='riproduzione'?' btn--ember':''}" data-fmode="riproduzione">${t('▶ Riproduzione')}</button>
    <button class="btn${fotoMode==='confronto'?' btn--ember':''}" data-fmode="confronto">${t('⚖ Confronto')}</button></div>`;
  let view='';
  if(fotoMode==='riproduzione'){
    ...
    view=`<div style="text-align:center;margin-top:8px">
      ...
      <div class="bar no-print" style="justify-content:center;gap:10px;margin-top:8px">
        <button class="btn" id="foto-pl-play" title="${t('Riproduci / pausa')}" style="min-width:42px">▶</button>
      ...
      ${!fotoTag?`<div class="muted no-print" style="font-size:11px;margin-top:2px">${t('Vista «tutti»: ogni passo mostra le viste della stessa data affiancate. Scegli una vista per scorrere una foto per volta.')}</div>`:''}</div>`;
  } else {
    ...
    view=`<div style="margin-top:8px">
      <div class="row no-print" style="gap:10px">
        <div class="field"><label>${t('📅 Prima (vecchia)')}</label><input type="date" id="foto-cmp-a" min="${dmin}" max="${dmax}" value="${fotoPrimaData}"></div>
        <div class="field"><label>${t('📅 Dopo (recente)')}</label><input type="date" id="foto-cmp-b" min="${dmin}" max="${dmax}" value="${fotoDopoData}"></div></div>
      <div class="muted no-print" style="font-size:11px;margin:-2px 0 6px">${t('Scegli due date dal calendario (si aggancia alla sessione più vicina). Con la vista «tutti» vedi le viste affiancate.')}</div>
      <div style="text-align:center"><div class="muted" style="font-size:11px;text-transform:uppercase;letter-spacing:.5px;margin:6px 0 2px">${t('Prima')}</div>
        ...
        <div class="muted" style="font-size:11px;text-transform:uppercase;letter-spacing:.5px;margin:12px 0 2px">${t('Dopo')}</div>
