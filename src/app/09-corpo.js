...
function renderCorpo(){
  ...
  const wSeries=[{name:t('Peso'),color:'var(--orange-b)',data:io.map((r,i)=>({x:lbl[i],y:+r.peso}))}];
  const bmiSeries=[{name:'BMI',color:'var(--violet)',data:io.map((r,i)=>({x:lbl[i],y:+r.bmi}))}];
  const massSeries=[
    {name:t('Massa magra'),color:'var(--ok)',data:io.map((r,i)=>({x:lbl[i],y:+r.massa_magra}))},
    {name:t('Massa musc.'),color:'var(--orange)',data:io.map((r,i)=>({x:lbl[i],y:+r.massa_muscolare}))}];
  const F=(k,lab,step,suf)=>`<tr><td class="l">${t(lab)}</td><td><input class="cell-in" type="number" step="${step||'any'}" data-u="${k}" value="${u[k]??''}" style="width:90px"></td><td class="muted l">${suf?t(suf):''}</td></tr>`;
  document.getElementById('panel-corpo').innerHTML=`
   ${statusBanner(DOC.storico_io,'Storico misure')}
   <div class="cards">
     <div class="card k--ember"><div class="card__k">${t('Peso')}</div><div class="card__v">${nf(u.peso,1)}<small> kg</small></div></div>
     ...
     <div class="card k--violet"><div class="card__k">${t('Fabbisogno')}</div><div class="card__v">${nf(c.metab,0)}<small> kcal</small></div><div class="card__sub">${t('basale·PAL')}</div></div>
   </div>
   ...
   <div class="chart-grid">
     <div>
       ...
       <div class="tbl-wrap"><table><tbody>
         ...
         <tr><td colspan="3" class="muted" style="font-size:11px">${t('Altezza, data di nascita, sesso e nome sono nel tab <b>Profilo</b> (dati invariati).')}</td></tr>
       </tbody></table></div>
     </div>
     <div>
       <div class="chart-box"><h4>${t('⚖ Peso')}</h4>${lineChart(wSeries,{labels:lbl,h:150})}</div>
       <div class="chart-box"><h4>BMI</h4>${lineChart(bmiSeries,{labels:lbl,h:150})}</div>
       <div class="chart-box"><h4>${t('Massa magra / muscolare')}</h4>${lineChart(massSeries,{labels:lbl,h:150})}</div>
     </div>
   </div>
   <div class="callout callout--info no-print"><div>${t('📜 Lo storico completo delle misure (')}${io.length}${t(' rilevazioni) è nella sezione «Misure», raggiungibile dal link nel footer.')}</div></div>
   <div id="foto-box"></div>`;
  ...
  document.getElementById('btn-undo-io').onclick=()=>{ if(!io.length){alert(t('Vuoto.'));return;}
    if(!confirm(t('Elimino l\'ultima misura salvata?')))return; DOC.storico_io.pop(); persist('corpo'); renderCorpo(); };
...
function saveIoModal(){
  ...
  modal(`<h3>${t('💾 Salva misure')}</h3>
    <div class="row"><div class="field"><label>${t('Anno')}</label><input id="m-anno" type="number" value="${w.anno}"></div>
      <div class="field"><label>${t('Settimana')}</label><input id="m-sett" type="number" min="1" max="53" value="${w.sett}"></div></div>
    <p class="muted" style="font-size:12.5px">${t('Verranno salvati: Peso')} ${nf(u.peso,1)} kg · BMI ${nf(c.bmi,1)} · ${t('Massa magra')} ${nf(c.magraKg,1)} kg.</p>
  ...
  document.getElementById('m-ok').onclick=()=>{
    ...
    if(DOC.storico_io.some(r=>(+r.scheda)===code) && !confirm(t('Esiste già la misura')+' '+code+t('. Aggiungo comunque?')))return;
