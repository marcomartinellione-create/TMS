...
**File coinvolti**: src/app/13d-cruscotto.js (nuovo) · src/app/09b-corpo-foto.js (nuovo) · src/app/06-allenamento.js + 01-costanti.js (LED co-pilota, `lastBlockPesi`) · src/app/13b-profilo.js (semaforo nelle righe) · src/app/03-persistenza.js + 14-backup.js (campo `foto` in corpo.json: load/save/backup/restore; `mimeDi` webp/gif) · src/app/09-corpo.js + 13-report.js (sezione foto + prima/dopo) · src/app/13a-guida.js + 13c-guida-ai.js + docs/guida-ai.md (guide) · src/manifest.json · tools/genera-foto-demo.js (nuovo, foto demo template) · package.json ×2 · tests/test-app.js
**Descrizione**:
- **Cruscotto multi-cliente** (tab 👤 Profilo, sola lettura): ogni profilo nella lista ha accanto al nome un **semaforo** 🟢/🟡/🔴/⚪ e una riga di sintesi (ACWR ultima settimana, da quante settimane non aggiorna la scheda, monotonia se RPE attivo, PR recenti) — triage di tutti i clienti senza aprirli uno per uno. Integrato nelle righe profilo (niente sezione separata, su richiesta di Marco), look invariato. Soglie: 🔴 ACWR>1.5 · scheda ferma ≥3 sett · monotonia>2 con ACWR>1.3; 🟡 ACWR 1.3–1.5 o <0.8 · 2 sett · monotonia>2. Riusa i calcoli esistenti scambiando temporaneamente `DOC` (ripristino garantito), legge i profili non attivi con `readProfileFull`. **Zero scrittura**.
- **Foto progressi** (tab 🜂 Corpo): foto datate salvate IN LOCALE in `TMS_Dati/<profilo>/foto/` (scrittura binaria via il ponte, come i video; nessun upload/cloud), metadati `{file,data,tag}` in `corpo.json`. Due modalità: **▶ Riproduzione** (timelapse, play/pausa; vista «tutti» = viste della stessa data affiancate, vista singola = una foto per volta) e **⚖ Confronto** (due date da **calendario** con aggancio alla sessione più vicina, vecchia sopra/recente sotto, con peso). Filtro per vista, gestione raggruppata per data. **Report**: nuova sezione «Foto progressi (prima/dopo)» con scelta delle date in compilazione, immagini incorporate in base64 (PDF + digitale offline). I backup JSON includono solo i metadati, non le immagini (come i video).
- **Co-pilota LED passivo** (scheda 🏋 Pesi): pallino 🟢/🟡/🔴 accanto alle frecce ▲▼ di ogni esercizio che valuta se il peso digitato è una progressione sensata, confrontandolo col set di pari posizione della scorsa scheda (`lastBlockPesi`) e coi segnali di affaticamento (ACWR, monotonia, RIR ultima volta). Soglie: 🟢 −5%…+10% · 🟡 +10%…+20% / calo oltre −5% / aumento con RIR≤1 / monotonia alta · 🔴 oltre +20% o aumento con ACWR>1.5 (meglio scaricare). Tooltip col motivo. **Solo visivo, nessuna scrittura automatica** (richiesta esplicita di Marco: niente generazione della scheda).
**Test**: `npm test` **244/244** (lint + sintassi + jsdom) — nuove verifiche: cruscotto (triage, soglie, sola lettura, DOC intatto), foto (scrittura binaria, metadati in corpo.json + backup, modalità a gruppi/calendario), LED (tutte le soglie, posizione, niente scrittura). Build md5 identici, `npm run verifica` OK. NB: rendering reale (immagini foto, PDF, LED dal vivo) collaudato in app/dev.
**Approvato da**: Marco (proposte approvate una alla volta + comando di pubblicazione «pubblichiamo»)
...
**File coinvolti**: **tools/genera-seed.js (NUOVO)** · electron/package.json (`predist` + extraResources → `seed/TMS_Dati`) · tools/genera-profilo-template.py (dieta nuova) · TMS_Dati/template/* (rigenerato) · src/app/{13-report.js (guida rapida), 01-costanti.js} · .gitignore · package.json ×2 (1.0.69) · CLAUDE.md
**1 · Seed dell'installer senza dati personali**: il pacchetto pubblico ora parte con il SOLO profilo "Atleta Template" (attivo). `tools/genera-seed.js` costruisce `seed/TMS_Dati/` (template + catalogo, profili.json riscritto) ed è agganciato a electron-builder via `predist`. **Nessuna perdita per nessuno**: `TMS_Dati/` del progetto resta intatta (dati di Marco inclusi), le installazioni esistenti conservano i loro dati in userData, e in dev il seed resta la root. Decisione di Marco (2026-06-11), che supera la scelta del 2026-06-10 di includere i dati reali.
**2 · Dieta del template realistica** (atleta 74,5 kg, TDEE ~2600): tre giornate complete con nomi USAV verificati e finestre asserite dal generatore — bulk 2830 kcal · mant 2412 · cut 1863, proteine ~150 g (≈2 g/kg), 14-15 alimenti per fase con olio, frutta, pesce, quark. I 3 periodi demo del tab Analisi usano la dieta nuova.
**3 · Guida rapida riscritta**: via il passo "connetti la cartella: è obbligatorio" (gate rimosso in v1.0.56!) → "apri l'app: è già pronta" con profilo demo; passi e tabella sezioni aggiornati a TUTTE le funzioni recenti (video personali, sottocategorie, scambio scheda, Periodi, tab Analisi, report digitale).
**4 · Repo GitHub come vetrina**: .gitignore rivisto (esclusi dati personali, database, generati, Doc) e push del sorgente decomposto su `main` (README con DOI e canale YouTube, CHANGELOG, src/, tools/, tests/, electron/) al posto dei file del canale dismesso.
**Test**: `npm test` 80/80 · seed verificato (solo template, active=template, 6 file) · finestre caloriche asserite nel generatore.
**Approvato da**: Marco (richiesta esplicita: "atleta template, dieta sensata, non eliminare i miei dati, guida rapida, vetrina git")
